...
INSTALLED_APPS = [
    'daphne',  # DaphneÎ•?Îß??ÑÏóê Ï∂îÍ? (ASGI ?úÎ≤Ñ)
    ...
    'rest_framework',
    'corsheaders',
    'channels',  # Channels Ï∂îÍ?
    'django_celery_beat',  # Celery Beat Ï∂îÍ?
    'django_celery_results',  # Celery Results Ï∂îÍ?
    'storages',  # ?åÏùº ?§ÌÜ†Î¶¨Ï? Ï∂îÍ?
    'api',  # Our API app
]
...
MIDDLEWARE = [
    ...
    'api.middleware.db_check.DatabaseConnectionMiddleware',  # DB ?∞Í≤∞ Ï≤¥ÌÅ¨ Ï∂îÍ?
    'corsheaders.middleware.CorsMiddleware',
    'django.contrib.sessions.middleware.SessionMiddleware',
    'django.middleware.common.CommonMiddleware',
    'django.middleware.csrf.CsrfViewMiddleware',  # CSRF ?§Ïãú ?úÏÑ±??
...
ASGI_APPLICATION = 'healthwise.asgi.application'  # ASGI Ï∂îÍ?
...
def get_database_config():
    ...
    # Railway?êÏÑú??buildtime??Reference Variable???¥ÏÑù?òÏ? ?äÏùå
    # ?∞Ì??ÑÏóê?úÎßå ?¨Ïö© Í∞Ä??
    ...
    if database_url:
        ...
        # ÎπåÎìú ?Ä?ÑÏù¥??Î°úÏª¨ Í∞úÎ∞ú???ÑÏãú ?§Ï†ï
        return {
...
AUTHENTICATION_BACKENDS = [
    'django.contrib.auth.backends.ModelBackend',  # Í∏∞Î≥∏ Django ?∏Ï¶ù
    'api.supabase_auth.SupabaseAuthBackend',  # Supabase ?∏Ï¶ù Ï∂îÍ?
]
...
if not DEBUG:
    ...
    CSRF_COOKIE_SAMESITE = 'None'  # Cross-origin ?îÏ≤≠???ÑÌï¥
    SESSION_COOKIE_SAMESITE = 'None'
    SESSION_COOKIE_HTTPONLY = False  # JavaScript?êÏÑú ?ëÍ∑º Í∞Ä?•Ìïò?ÑÎ°ù
    CSRF_COOKIE_HTTPONLY = False  # JavaScript?êÏÑú ?ëÍ∑º Í∞Ä?•Ìïò?ÑÎ°ù
    
    # ?∏ÏÖò Ïø†ÌÇ§Í∞Ä ?úÎ?Î°??§Ï†ï?òÎèÑÎ°??ÑÎ©î???§Ï†ï
    SESSION_COOKIE_DOMAIN = None  # ?îÏ≤≠ ?ÑÎ©î???êÎèô ?¨Ïö©
    CSRF_COOKIE_DOMAIN = None  # ?îÏ≤≠ ?ÑÎ©î???êÎèô ?¨Ïö©
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': [
        'api.jwt_auth.CustomJWTAuthentication',  # JWT ?∏Ï¶ù ?∞ÏÑ†
        'api.supabase_auth.SupabaseJWTAuthentication',  # Supabase JWT ?∏Ï¶ù Ï∂îÍ?
    ...
    'DEFAULT_PARSER_CLASSES': [
        ...
        'rest_framework.parsers.MultiPartParser',  # ?åÏùº ?ÖÎ°ú?úÎ? ?ÑÌï¥ Ï∂îÍ?
...
# API Keys (?òÍ≤ΩÎ≥Ä?òÏóê??Í∞Ä?∏Ïò§Í∏?
...
SESSION_COOKIE_NAME = 'healthwise_sessionid'  # Ïª§Ïä§?Ä ?∏ÏÖò Ïø†ÌÇ§ ?¥Î¶Ñ
