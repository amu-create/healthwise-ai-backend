# social/views/__init__.py에서 import된 social_views.py
# 기존 views.py의 내용을 이동
...
class StoryViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        try:
            ...
            logger.info(f"Story upload attempt by user: {self.request.user.username}")
            ...
            if 'media_file' in serializer.validated_data:
                ...
                # content_type 안전하게 가져오기
                content_type = ''
                if hasattr(media_file, 'content_type'):
                    content_type = media_file.content_type
                elif hasattr(media_file, 'file') and hasattr(media_file.file, 'content_type'):
                    content_type = media_file.file.content_type
                
                logger.info(f"Media file content_type: {content_type}")
            ...
            logger.error(f"Story creation error: {str(e)}", exc_info=True)
            raise
